...
const RippleCursor: FC<RippleCursorProps> = ({
  maxSize = 50,
  duration = 1000,
  blur = true,
}) => {
  ...
  return (
    <div className='fixed top-0 left-0 w-screen h-screen pointer-events-none overflow-hidden z-[-1]'>
      {ripples.map((ripple) => (
        <div
          key={ripple.id}
          className='absolute rounded-full bg-blue-500 bg-opacity-10 shadow-[0_0_10px_rgba(0,150,255,0.3),0_0_20px_rgba(0,150,255,0.4)] animate-ripple'
          style={{
            left: `${ripple.x}px`,
            top: `${ripple.y}px`,
            width: `${maxSize}px`,
